...
export async function fetchAniListUserBoth(username, progressCallback = null) {
	try {
		...
		const isekaiAnime = allMediaList
			.filter(entry => {
				const isCompleted = entry.status === 'COMPLETED';
				const hasIsekaiTag = entry.media.tags?.some(tag => tag.name.toLowerCase() === 'isekai' && tag.rank >= 80);

				if (hasIsekaiTag) {
					console.log(
						`🌍 Found isekai anime: "${entry.media.title.english || entry.media.title.romaji}" with Isekai tag rank: ${entry.media.tags.find(t => t.name.toLowerCase() === 'isekai')?.rank}%`
					);
				}
